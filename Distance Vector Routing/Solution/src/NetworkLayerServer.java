...
//        }
    ...
    public static synchronized void DVR(int startingRouterId) {
        ...
        */
        ...
        {
            convergence=false;
            for (int routerIndex=routers.indexOf(startingRouter),i=0;i<routers.size();i++)
            {
                Router r=routers.get(routerIndex);
                for (int j=0;j<r.getNeighborRouterIDs().size();j++)
                {
                    Router t=routerMap.get(r.getNeighborRouterIDs().get(j));
                    if (!t.getState())
                    {
                        continue;
                    }
                    boolean result=t.sfupdateRoutingTable(r);
                ...
                routerIndex++;
                routerIndex=routerIndex%routers.size();
            }
        }
        ...
        while (convergence)
        {
            ...
            for (int routerIndex=routers.indexOf(startingRouter),i=0;i<routers.size();i++)
            {
                ...
                for (int j=0;j<r.getNeighborRouterIDs().size();j++)
                {
                    ...
                    boolean result=t.updateRoutingTable(r);
                    if(result&&!convergence)
                    {
                        convergence=true;
                    }
